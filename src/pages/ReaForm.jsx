import { useState } from "react";
import { Save } from "lucide-react";
import Header from "../components/Header";
import FileUpload from "../components/FileUpload";
import HistoryPanel from "../components/HistoryPanel";

import {
  FormInput,
  FormSelect,
  FormReadOnly,
  FormTextarea,
  SectionDivider,
} from "../components/forms/FormComponents";

const SEMESTRES = ["2026-1", "2026-3", "2027-1", "2027-3", "2028-1", "2028-3"];

const STATUS_OPTIONS = [
  "Selecione o Status atual",
  "Pendente",
  "Em análise",
  "Aprovado",
  "Reprovado",
  "Concluído",
  "Cancelado",
];

const CURSOS = [
  "",
  "Análise e Desenvolvimento de Sistemas",
  "Gestão Comercial",
  "Administração",
  "Design Gráfico",
];

const TIPOS_AVALIACAO = [
  "",
  "Prova substitutiva",
  "Atividade avaliativa",
  "Trabalho avaliativo",
  "Outro",
];

const TURNOS = ["", "Manhã", "Tarde", "Noite"];

const ESTADO_INICIAL = {
  semestre: "2026-1",
  status: "Selecione o Status atual",
  componente: "",
  curso: "",
  professor: "",
  tipoAvaliacao: "",
  turno: "",
  instrucoes: "",
  criadoPor: "",
  dataCriacao: "",
  modificadoPor: "",
  dataModificacao: "",
};

export default function ReaForm({ voltar, salvarFicha }) {
  const [form, setForm] = useState(ESTADO_INICIAL);

  function handleChange(campo) {
    return (e) => setForm((prev) => ({ ...prev, [campo]: e.target.value }));
  }

  function salvarSolicitacao() {
    const obrigatorios = [
      form.componente,
      form.curso,
      form.professor,
      form.tipoAvaliacao,
      form.turno,
    ];

    const invalido =
      obrigatorios.some((campo) => !campo.trim()) ||
      form.status === "Selecione o Status atual";

    if (invalido) {
      alert(
        "Preencha componente curricular, curso, professor, tipo de avaliação, turno e status antes de salvar."
      );
      return;
    }

    salvarFicha({
      processo: "REA",
      semestre: form.semestre,
      idAluno: "-",
      aluno: form.componente,
      curso: form.curso,
      status: form.status,
    });
  }

  return (
    <div className="min-h-screen bg-gray-100">
      <Header
        title="Regime Especial de Avaliação"
        subtitle="REA | Nova solicitação"
        onBack={voltar}
      />

      <main className="p-10">
        <section className="bg-white rounded-2xl shadow-lg border border-gray-200 max-w-6xl mx-auto">
          <div className="border-b border-gray-200 p-6">
            <h2 className="text-xl font-bold text-gray-800">
              Dados da solicitação
            </h2>
            <p className="text-gray-500 text-sm mt-1">
              Preencha as informações abaixo para registrar a avaliação.
            </p>
          </div>

          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormSelect
              label="Semestre"
              options={SEMESTRES}
              value={form.semestre}
              onChange={handleChange("semestre")}
            />

            <FormReadOnly label="Processo" value="REA" />

            <FormSelect
              label="Status"
              options={STATUS_OPTIONS}
              value={form.status}
              onChange={handleChange("status")}
            />

            <SectionDivider title="Dados acadêmicos" />

            <FormInput
              label="Componente curricular"
              type="text"
              colSpan2
              value={form.componente}
              onChange={handleChange("componente")}
              placeholder="Digite o nome da disciplina"
            />

            <FormSelect
              label="Curso"
              options={[
                { value: "", label: "Selecione o curso" },
                ...CURSOS.filter(Boolean).map((curso) => ({
                  value: curso,
                  label: curso,
                })),
              ]}
              value={form.curso}
              onChange={handleChange("curso")}
            />

            <FormInput
              label="Professor(a)"
              type="text"
              value={form.professor}
              onChange={handleChange("professor")}
              placeholder="Nome ou e-mail do professor"
            />

            <FormSelect
              label="Tipo de avaliação"
              options={[
                { value: "", label: "Selecione o tipo" },
                ...TIPOS_AVALIACAO.filter(Boolean).map((tipo) => ({
                  value: tipo,
                  label: tipo,
                })),
              ]}
              value={form.tipoAvaliacao}
              onChange={handleChange("tipoAvaliacao")}
            />

            <FormSelect
              label="Turno"
              options={[
                { value: "", label: "Selecione o turno" },
                ...TURNOS.filter(Boolean).map((turno) => ({
                  value: turno,
                  label: turno,
                })),
              ]}
              value={form.turno}
              onChange={handleChange("turno")}
            />

            <FormTextarea
              label="Instruções"
              rows={5}
              colSpan2
              value={form.instrucoes}
              onChange={handleChange("instrucoes")}
              placeholder="Insira orientações específicas para a aplicação da avaliação"
            />

            <div className="md:col-span-2">
              <FileUpload />
            </div>

            <SectionDivider title="Registro de funcionário" />

            <FormInput
              label="Criado por"
              type="text"
              value={form.criadoPor}
              onChange={handleChange("criadoPor")}
              placeholder="Usuário responsável pela criação"
            />

            <FormInput
              label="Data de criação"
              type="date"
              value={form.dataCriacao}
              onChange={handleChange("dataCriacao")}
            />

            <FormInput
              label="Modificado por"
              type="text"
              value={form.modificadoPor}
              onChange={handleChange("modificadoPor")}
              placeholder="Último usuário que modificou"
            />

            <FormInput
              label="Data de modificação"
              type="date"
              value={form.dataModificacao}
              onChange={handleChange("dataModificacao")}
            />

            <div className="md:col-span-2">
              <HistoryPanel />
            </div>
          </div>

          <div className="border-t border-gray-200 p-6 flex flex-col md:flex-row justify-end gap-3">
            <button
              onClick={voltar}
              className="px-5 py-3 rounded-xl border border-gray-300 text-gray-700 hover:bg-gray-100 transition"
            >
              Cancelar
            </button>

            <button
              onClick={salvarSolicitacao}
              className="px-5 py-3 rounded-xl bg-blue-900 text-white hover:bg-blue-800 transition flex items-center justify-center gap-2"
            >
              <Save size={18} />
              Salvar solicitação
            </button>
          </div>
        </section>
      </main>
    </div>
  );
}
